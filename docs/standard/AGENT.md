...
2. **Assess the state of the documentation:**
   ...
   - If only `vision.md`, `roadmap.md`, and `tech_stack.yaml` exist and are complete, and the root `AGENTS.md` declares `Adoption profile: lite` → the project uses **Lite Mode** (see the Adoption Guide in `README.md`). The missing files are intentional: do not demand them and do not enter Onboarding Mode. User Stories live directly on the roadmap board. If you detect an upgrade signal (a board item accumulating heavy functional detail, a cross-cutting process, re-litigated technology decisions), report it to the user and recommend the upgrade path — do not upgrade on your own initiative.
